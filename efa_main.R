...
analyze_efa <- function(data_obj,
  ...
  sensitivity_exclude_items <- unlist(efa_config$sensitivity_exclude_items)
  ...
  for (subset_info in exclude_subsets) {
    ...
    for (n in n_factors_list) {
      ...
      cat("\n========================================\n")
      cat(sprintf("EFA ANALYSIS [%s]: n_factors = %d\n", subset_key, n))
      ...
      for (fm in extraction_methods) {
        ...
        cat(sprintf("\nStep 3 [%s | n=%d | %s]: Factor Extraction and Rotation (Polychoric)\n",
                    subset_key, n, fm))
        cat("----------------------------------------\n")
        ...
        cat(sprintf("\nStep 4 [%s | n=%d | %s]: Factor Extraction and Rotation (Pearson)\n",
                    subset_key, n, fm))
        cat("----------------------------------------\n")
        cat("Extracting", n, "factors...\n")
        ...
        cat(sprintf("\nStep 5 [%s | n=%d | %s]: Aligning Pearson solution to Polychoric\n",
                    subset_key, n, fm))
        cat("------------------------------------------------\n")
      ...
      cat(sprintf("\nStep 7 [%s | n=%d]: Export pattern matrices to CSV\n", subset_key, n))
      cat("----------------\n")
      ...
      # Include subset_key in dataset_name for file naming
      dataset_name_with_subset <- paste0(dataset_name, "_", subset_key)
      ...
      for (fm in extraction_methods) {
        for (gamma in gamma_values) {
          ...
          export_pattern_csv(poly_pattern, output_dir, dataset_name_with_subset, fm,
                             "oblimin", gamma, n, "polychoric")
          export_pattern_csv(pear_pattern, output_dir, dataset_name_with_subset, fm,
    ...
    all_results$n_factors_list <- n_factors_list
    all_results$data <- data_fa
    all_subset_results[[subset_key]] <- all_results
  ...
  # Build return value
  results <- all_subset_results
  results$exclude_subsets <- exclude_subsets
  ...
  results$config_used <- list(
    ...
    pd_tolerance = pd_tol,
    display_cutoff = display_cutoff,
    sensitivity_exclude_items = sensitivity_exclude_items
  )
  
  invisible(results)
}

# Function to display EFA evaluation (all subsets, n_factors and extraction methods)
show_efa_evaluation <- function(data_obj) {
  ...
  exclude_subsets <- results$exclude_subsets
  ...
  for (subset_info in exclude_subsets) {
    ...
    cat("\n################################################################\n")
    cat(sprintf("EVALUATION SUBSET: %s\n", subset_key))
    if (length(subset_info$items) > 0) {
      cat(sprintf("  Excluded items: %s\n", paste(subset_info$items, collapse = ", ")))
    } else {
      cat("  Excluded items: none (full dataset)\n")
    }
    cat("################################################################\n")
        ...
        for (set_name in names(all_evaluations)) {
          ...
          if (set_info$evaluation$n_failed == 0) {
            ...
            if (set_info$rotation_type == "oblimin") {
              ...
              filename <- sprintf("pattern_%s_n%d_%s_%s_Oblimin_gamma_%.2f.csv",
                                  subset_key, n, fm, set_info$cor_type, set_info$param_value)
            } else {
              kappa_key <- paste0("kappa_", set_info$param_value)
              pattern <- set_info$results$efa$rotations_promax[[kappa_key]]$pattern
              filename <- sprintf("pattern_%s_n%d_%s_%s_Promax_kappa_%d.csv",
                                  subset_key, n, fm, set_info$cor_type, set_info$param_value)
            }
